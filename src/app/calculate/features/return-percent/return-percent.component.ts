...
export class ReturnPercentComponent {
  ...
  calculate() {
    ...
    if(ReturnTypeEnum.PERCENTAGE === this.returnType) {
      ...
      setTimeout(() => {
        this.displayValue(this.percentageValueEl.nativeElement, this.percentageValue);
      }, 0);
      ...
      setTimeout(() => {
        this.displayValue(this.interestValueEl.nativeElement, this.interestValue);
        this.displayValue(this.totalValueEl.nativeElement, this.totalValue);
      }, 0);
    }
  }
  ...
  displayValue(element: any, finalValue: number) {
    ...
    let id = setInterval(() => {
      if((isPositive && displayValue < finalValue || (!isPositive && displayValue > finalValue))) {
        displayValue = this.roundToTwoDigits(displayValue + incrementValue);
