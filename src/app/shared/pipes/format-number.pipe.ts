...
export class FormatNumberPipe implements PipeTransform {
  ...
  transform(
    ...
    if(options.symbol === '₹') {
      ...
      if(currencyValue.toString().length >= 6) {
        let formattedCurrency = this.getFormatedCurrency(currencyValue, options.decimalPoint); 
  ...
  formatToIndianNumberingSystem(number: number,  decimalPoint: number = 2) {
    let numberString = this.removeZeroAfterDecimal(number.toFixed(decimalPoint).toString());
